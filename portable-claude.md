...
- **No `cd "$(git rev-parse --show-toplevel)"`:** Don't wrap git (or any) commands in a `cd "$(...)"` to the repo root. The command substitution forces a permission prompt even for allowlisted commands. Run git directly from cwd, or use `git -C <path>`.
