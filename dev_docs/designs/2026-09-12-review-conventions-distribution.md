...
- **This plugin carries none.** `rg -i review portable.md portable-claude.md` returns four lines; three are coding rules mentioning review in passing, one routes technical-correctness questions to agent review. None says how to review or how to write a finding.
...
- Whether `crush` reliably consumes `<INPUT>` on stdin at all. Observed 2026-09-12 during the co-review of this document: `crush` v0.92.0 returned exactly `NO INPUT` on a stdin-piped input that `codex` read successfully on the same run. `NO INPUT` is the string its own pointer instructs it to emit when stdin is empty, so this is crush reporting an empty read, not a model refusal. If it reproduces, decision 3 does not reach `crush`.
