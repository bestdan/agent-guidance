#!/usr/bin/env python3
"""Tests hooks/gh-body-guard.sh. Run by gh-body-guard.test.sh.

Reads the guard under test from GUARD and the repo root from ROOT, both set by
the wrapper, and prints one ok/FAIL line per case plus a failure count.

The suite is a file rather than a `python3 -c` argument for the reason its own
subject demands: every case here is about a quote, and inside a single-quoted
shell argument this file could not write one. The cases used to spell
`chr(39)`, `chr(34)` and `chr(92)` for characters they are asserting on.
`dev_docs/decisions/2026-09-20-hook-python-lives-in-a-file-beside-the-shell.md`
is the record; it covers the hooks, and this file is the same move.
"""
...
check("line continuation inside the flag word",
      "deny", run('gh pr comment 1 --bo\\\ndy "`date`"'))

check("ordinary multi-line continuation",
      "deny", run('gh pr comment 1 \\\n  --body "`date`"'))
...
check("gh nested in a substitution",
      "deny", run('echo $(gh pr comment 1 --body "`date`")'))
...
check("single-quoted title is literal",
      "allow", run("gh pr create --title 'fix `date` handling' --body-file b.md"))
...
check("-b belongs to the other command in the compound",
      "allow", run('sort -b "$(cat list)" && gh pr view 1'))

check("--body belongs to the other command in the compound",
      "allow", run('printf --body "$(id)" && gh pr view 1'))

check("body flag before gh, separated by a semicolon",
      "allow", run('echo --body "`date`"; gh pr view 1'))

check("trailing comment is not live text",
      "allow", run('gh pr view 1 # --body "`date`"'))

check("single-quoted backtick is literal",
      "allow", run("gh pr comment 1 --body 'see `date` here'"))

check("single-quoted $( is literal",
      "allow", run("gh pr comment 1 --body 'cost $(x)'"))
...
# A wrapper whose program is gone. The never-block rule is unconditional, so
# the wrapper must still exit 0 and print nothing, which means the guard fails
# open: this is the cost the extraction accepts, measured rather than assumed.
# A wrapper that propagated the python exit status instead would block every
# Bash call in every session that installed the plugin, so the case is here to
# make that regression impossible to land quietly. The copy carries the wrapper
# alone, and the payload clears both case prefilters so python is really reached.
lone_dir = tempfile.mkdtemp(prefix="gh-body-guard-lone.")
lone = os.path.join(lone_dir, "gh-body-guard.sh")
shutil.copy(guard, lone)
lone_payload = json.dumps({
    "hook_event_name": "PreToolUse",
    "tool_name": "Bash",
    "session_id": "test",
    "tool_input": {"command": "gh pr comment 1 --body \"see `date`\""},
})
lone_proc = subprocess.run(
    ["bash", lone], input=lone_payload, capture_output=True, text=True
)
shutil.rmtree(lone_dir, ignore_errors=True)
check("a wrapper with no program exits 0", 0, lone_proc.returncode)
check("a wrapper with no program says nothing", "", lone_proc.stdout.strip())
